...
root_suffix='.root'
nDirectories=${#output_files[@]} 
echo "There are $nDirectories directories to process"

for index in ${!input_files[*]}; do
	echo "Beginning to process number $index: ${output_files[$index]}"
	condor_submit \
		arg1=${input_files[$index]} \
		arg2=${output_files[$index]}${root_suffix} \
		arg3=${nDirectories} \
